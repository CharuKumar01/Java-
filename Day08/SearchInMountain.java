...
public class SearchInMountain {
    ...
    static int search(int[] arr, int target){
        ...
        int firstTry = orgerAgnosticBS(arr, target, 0, peak);
        if (firstTry != -1) {
            return firstTry;
        }
        // try to search in second half
        return orgerAgnosticBS(arr, target, peak+1, arr.length -1);
    }
    ...
    static int orgerAgnosticBS(int[] arr, int target, int start, int end) {
